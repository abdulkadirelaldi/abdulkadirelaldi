...
/**
 * T-020 — `LogoItem` iki bicimli bir birlesim; kaynak kod bunu `as any` ile
 * geciyordu. §2 `any` kullanimini YASAKLIYOR, o yuzden gercek tip daraltmasi
 * kullaniliyor: hangi alanin var oldugu derleyici tarafindan biliniyor.
 */
  ...
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        ...
        const nodeItem: LogoNodeItem | null = isNodeLogo(item) ? item : null;
        const imageItem: LogoImageItem | null = isNodeLogo(item) ? null : item;
        ...
        const content = nodeItem ? (
          <span
            ...
            aria-hidden={Boolean(nodeItem?.href) && !nodeItem?.ariaLabel}
          >
            {nodeItem?.node}
          </span>
        ) : (
          /*
           * next/image KULLANILMIYOR: logolar cagiran tarafin verdigi RASTGELE
           * uzak adreslerden gelir ve `next/image` her alan adi icin
           * `next.config.ts` icinde `remotePatterns` tanimi ister — o dosya
           * ortak (§10.1) ve bu bilesenin kontrolunde degil. Logolar kucuk
           * (yukseklik ~28px) ve `loading="lazy"`; LCP adayi degiller.
           * Logolar ileride repoya tasinirsa next/image'a gecilmeli.
           */
